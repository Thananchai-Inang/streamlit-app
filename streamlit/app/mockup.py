...
mongo_collection = db.your_collection
...
for year in range(2020,2025,1):
    for month in range(1,13,1):
        for date in range(1,M_DAYS[month],1):
            data_list.append({'Name':'UBU','Date':date,'Month':month,'Year':year,'WaterDataFront':random.randrange(100,200,1),'WaterDataBack':random.randrange(90,180,1),'WaterDrainRate':random.randrange(90,150,2)})
        if month==2 and isleap(year):
            data_list.append({'Name':'UBU','Date':29,'Month':month,'Year':year,'WaterDataFront':random.randrange(100,200,1),'WaterDataBack':random.randrange(90,180,1),'WaterDrainRate':random.randrange(90,150,2)})
...
for collection in data_list:
    print('[!] Inserting - ', collection)
    mongo_db.waterdata.insert_many(collection)